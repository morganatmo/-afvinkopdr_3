...
#om het allemaal overzichtelijker en mooier te maken importeer ik een tabellen-maker
from prettytable import PrettyTable

#een lijst met alle opgegeven enzymen erin
knip_enzymen = [["Desulfovibrio desulfuricans(DdeI) ", DdeI], ["Klebsiella pneumoniae(BspMII) ", BspMII], ["Escherichia coli(EcoRI) ", EcoRI], ["Haemophilus influenzae(HindIII) ", HindIII], ["Thermus aquaticus(TaqI) ", TaqI]]

#invullen van de tabel die voorheen al was opgeroepen
Tabel = PrettyTable (["enzym", "sequentie", "knipt"])
for enzym in knip_enzymen:
    if enzym[1] in sequentie.upper():
        Tabel.add_row([enzym[0], enzym[1], "wel"])              #de enzym komt dan wel in de sequentie voor
    else:
        Tabel.add_row([enzym[0], enzym[1], "niet"])             #de enzym komt dan niet in de sequentie voor
